...
local function getSkillViewCard(card, class_name, player, card_place)
	...
	elseif class_name == "Jink" then
		if player:hasSkill("baichux") then
			local jis = player:getPile("jix")
			jis = sgs.QList2Table(jis)
			local baichux_card = sgs.Sanguosha:getCard(jis[1])
			if card:getNumber() >= baichux_card:getNumber() then
				return ("jink:baichux[%s:%s]=%d"):format(suit, number, card_id)
			end
		end
		if player:hasSkill("longhun") and player:getHp() <= 1 then
...
function SmartAI:getAoeValueTo(card, to , from)
	...
	if self:aoeIsEffective(card, to) then
		...
		if card:inherits("ArcheryAttack") then
			...
			if to:hasSkill("qingguo") or self:isEquip("EightDiagram", to) or to:hasSkill("baichux") then
				value = value + 10
			end
		end
...
dofile "lua/ai/huangjin-ai.lua"
dofile "lua/ai/ubunmod-list.lua"
dofile "lua/ai/assassin-ai.lua"
dofile "lua/ai/artofwar-ai.lua"
